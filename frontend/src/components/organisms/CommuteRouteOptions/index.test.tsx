...
test('Test -Checkbox', () => {
  ...
  const role = screen.getByTestId("jobcard-role");
  screen.debug(undefined,Infinity);
  const company = screen.getAllByText(/BMW/)[0];
  ...
  const bus = screen.getByTestId("DirectionsBus");
  const bike = screen.getByTestId("PedalBike");
  const train = screen.getByTestId("TrainOutlined");
